...
SELECTED_ALGO_COLOR = (100, 100, 100)  # Màu cho thuật toán được chọn

# Fonts
TITLE_FONT = pygame.font.SysFont("Arial", 40, bold=True)
BUTTON_FONT = pygame.font.SysFont("Arial", 20)  # Giảm kích thước font cho bảng
...
def ida_star(start_state, goal_state):
    def search(state, g, threshold, path, nodes_expanded):
        ...
        next_states = sorted(get_next_states(state), key=lambda x: heuristic(x, goal_state))
        for next_state in next_states:
            if next_state not in path:  # Kiểm tra trong path để giảm bộ nhớ
                nodes_expanded[0] += 1
...
def simple_hill_climbing(start_state, goal_state, max_steps=500):
    ...
    for _ in range(max_steps):
        ...
        if not found_better:
            ...
            if unvisited and random.random() < 0.1:  # 10% xác suất để thoát cực trị cục bộ
...
def steepest_ascent_hill_climbing(start_state, goal_state, max_steps=500):
    ...
    for _ in range(max_steps):
        ...
        if best_state != state:
            ...
            # Chọn ngẫu nhiên để thoát cực trị cục bộ
            unvisited = [s for s in next_states if s not in visited]
...
def and_or_search(start_state, goal_state):
    ...
    # Sử dụng tìm kiếm OR với ưu tiên theo heuristic
    ...
    while stack:
        ...
        if state not in visited:
            visited.add(state)
            next_states = get_next_states(state)
            # Sắp xếp theo heuristic để ưu tiên trạng thái gần mục tiêu hơn
            next_states.sort(key=lambda x: heuristic(x, goal_state))
            for next_state in reversed(next_states):
                if next_state not in visited:
...
def belief_state_search(initial_belief_states, goal_state):
    ...
    visited = {initial_belief: 0}  # Lưu chi phí g nhỏ nhất cho từng belief state
    nodes_expanded = 0
...
def backtracking_search(start_state, goal_state):
    ...
    def backtrack(state, path, depth):
        ...
        # Sắp xếp các trạng thái lân cận theo heuristic để ưu tiên trạng thái gần mục tiêu
        next_states.sort(key=lambda x: heuristic(x, goal_state))
...
def genetic_algorithm(start_state, goal_state, population_size=50, generations=100, mutation_rate=0.1):
    ...
    def fitness(moves):
        ...
        h = heuristic(state, goal_state)
        # Thêm phạt nếu có chuỗi di chuyển lặp lại
    ...
    for _ in range(generations):
        # Đánh giá fitness và chọn lọc
        population = sorted(population, key=fitness)
        if fitness(population[0]) < best_fitness:
            best_individual = population[0]
            best_fitness = fitness(best_individual)
        
        # Kiểm tra nếu tìm thấy lời giải
        final_state = apply_moves(start_state, best_individual)
        nodes_expanded += population_size  # Mỗi thế hệ mở rộng population_size cá thể
        ...
        # Tạo thế hệ mới
        new_population = population[:population_size // 2]  # Giữ lại một nửa tốt nhất
    ...
    # Nếu không tìm thấy lời giải, trả về đường đi tốt nhất tìm được
    path = [start_state]
    current = start_state
    for move in best_individual:
        next_state = apply_moves(current, [move])
    ...
    visited = set()
    nodes_expanded = [0]
    path, success = search(start_state, [start_state], visited, nodes_expanded, False)
...
def ac3_search(start_state, goal_state):
    ...
    # Chuyển 8-Puzzle thành bài toán CSP
    variables = list(range(9))  # Các ô từ 0 đến 8
    domains = {var: set('012345678') for var in variables}  # Mỗi ô có thể chứa giá trị từ 0-8
    constraints = [(i, j) for i in range(9) for j in range(i + 1, 9)]  # Ràng buộc: các ô khác nhau phải có giá trị khác nhau

    # Áp dụng trạng thái ban đầu và mục tiêu
    initial_assignment = {i: start_state[i] for i in range(9)}
    goal_assignment = {i: goal_state[i] for i in range(9)}
    ...
    def ac3():
        ...
        while queue:
            ...
            if revise(xi, xj, domains):
                if not domains[xi]:
                    return False
                neighbors = [x for x in variables if x != xj and (x, xi) in constraints]
                queue.extend([(x, xi) for x in neighbors])
        return True

    # Áp dụng AC-3 để thu hẹp miền giá trị
    for var in variables:
        domains[var] = {initial_assignment[var]}  # Gán trạng thái ban đầu
    if not ac3():
        return None, 0, 0, 0

    # Sử dụng tìm kiếm để tìm đường đi
    queue = [(start_state, [start_state], 0)]
    visited = {start_state}
    nodes_expanded = 0

    while queue:
        state, path, nodes = queue.pop(0)
        if state == goal_state:
            return path, nodes, len(path) - 1, len(path) - 1
        next_states = get_next_states(state)
        ...
        for next_state in next_states:
            if next_state not in visited:
                # Kiểm tra xem next_state có thỏa mãn miền giá trị đã thu hẹp không
                valid = True
                for i in range(9):
                    if next_state[i] not in domains[i]:
                        valid = False
                        break
                if valid:
                    visited.add(next_state)
                    queue.append((next_state, path + [next_state], nodes_expanded))
...
def draw_algorithm_grid(screen, algorithms, selected_algorithm, mouse_pos):
    algo_buttons = []
    cols, rows = 3, 6  # 3 cột, 6 hàng để chứa 17 thuật toán
    button_width, button_height = 70, 25  # Giảm kích thước nút
    padding_x, padding_y = 4, 4  # Giảm khoảng cách giữa các nút
    start_x, start_y = 30, 270  # Dịch bảng sang trái và xuống dưới một chút
    ...
    algo_font = pygame.font.SysFont("Arial", 14)  # Font nhỏ hơn cho tên thuật toán
...
solve_rect = pygame.Rect(650, 350, 200, 40)
reset_rect = pygame.Rect(650, 400, 200, 40)
shuffle_rect = pygame.Rect(650, 450, 200, 40)

clock = pygame.time.Clock()

def reset_game():
    global start_input, goal_input, start_state, goal_state, path, current_step, current_state, auto_play, solved, solving, is_solving, timer_start, elapsed_time, nodes_expanded, search_depth, path_cost, total_steps, is_shuffled
...
def solve_puzzle():
    global is_solving, start_state, goal_state, solving, path, nodes_expanded, search_depth, path_cost, total_steps, current_step, current_state, timer_start, last_step_time, auto_play, solved
    if is_shuffled and not show_message_box("Please reset before solving!"):
        return
    is_solving = True
    start_state, goal_state = start_input, goal_input
    solving = True
    # Nếu thuật toán là Belief State Search, truyền một tập hợp chứa start_state
    ...
    timer_start = last_step_time = time.time()
# Main loop
# Cập nhật vị trí của các thành phần
start_rect = pygame.Rect(50, 150, 180, 40)  # Dịch lên trên
goal_rect = pygame.Rect(50, 220, 180, 40)   # Dịch lên trên
solve_rect = pygame.Rect(650, 400, 180, 40) # Thẳng hàng bên phải
reset_rect = pygame.Rect(650, 450, 180, 40) # Ngay dưới "Solve Puzzle"
shuffle_rect = pygame.Rect(650, 500, 180, 40) # Ngay dưới "Reset"

# Trong vòng lặp chính, cập nhật cách vẽ
while running:
    clock.tick(60)
    ...
    # Vẽ "Initial state" và "Goal state"
    ...
    # Vẽ bảng thuật toán
    draw_algorithm_grid(screen, algorithms, selected_algorithm, mouse_pos)
    
    # Vẽ bảng kết quả (dịch lên trên để thẳng hàng với puzzle)
