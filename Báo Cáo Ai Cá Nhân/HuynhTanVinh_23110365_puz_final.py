...
def ac3_search(start_state, goal_state):
    ...
    domains = {var: set('012345678') for var in variables}
    constraints = [(i, j) for i in range(9) for j in range(i + 1, 9)]
    ...
    for var in variables:
        domains[var] = {initial_assignment[var]}
    if not ac3():
        return None, 0, 0, 0
...
# Main loop
# Inside the while running loop (full context to ensure display)
while running:
    clock.tick(60)
    # Fill the background first to ensure it displays
    screen.fill(BG_COLOR)  # Ensure background is drawn
    ...
    # Draw all elements
    draw_board(current_state, screen)  # Draw the puzzle board
    ...
    # Display comparison chart if active
